...
contract ActivityScripts is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    /**
     * @dev Get all activity IDs
     * @return activityIds Array of all activity IDs
     */
    function getAllActivityIds() external view returns (string[] memory activityIds) {
        return allActivityIds;
    }
    
    /**
     * @dev Get total number of activities
     * @return count Total number of activities
     */
    function getActivityCount() external view returns (uint256 count) {
        return allActivityIds.length;
    }
    
    /**
     * @dev Get activities by author
     * @param authorId Author ID (Avatar ID or House ID)
     * @return activityIds Array of activity IDs created by the author
     */
    function getActivitiesByAuthor(string memory authorId) external view returns (string[] memory activityIds) {
        return activitiesByAuthor[authorId];
    }
    ...
    function createActivityScript(
        ...
        _scriptIds++;
        string memory activityId = string(abi.encodePacked("activity-", _scriptIds));
        ...
        // Add to enumeration arrays for efficient queries
        allActivityIds.push(activityId);
        activitiesByType[activityType].push(activityId);
        activitiesByPower[primaryPower].push(activityId);
        
        // Add to secondary powers
        for (uint256 i = 0; i < secondaryPowers.length; i++) {
            activitiesByPower[secondaryPowers[i]].push(activityId);
        }
        
        // Track by author
        activitiesByAuthor[authorId].push(activityId);
    ...
    function completeActivity(
        ...
        // Time limit means the activity must be completed WITHIN the time limit
        // (i.e., at or before createdAt + timeLimit)
        if (script.timeLimit > 0) {
            require(block.timestamp <= script.createdAt + script.timeLimit, "Activity time limit exceeded");
        }
     ...
     * @return activityIds Array of activity IDs (includes both primary and secondary power matches)
     */
    function getActivitiesByPower(PowerType powerType) external view returns (string[] memory activityIds) {
        return activitiesByPower[powerType];
    }
     ...
     * @dev Note: This iterates through all activities, which may be gas-intensive for large numbers
     *      Consider using events for off-chain indexing for better performance
     */
    function getAvatarCompletions(string memory avatarId) external view validAvatarId(avatarId) returns (string[] memory activityIds) {
        string[] memory completed = new string[](allActivityIds.length);
        uint256 count = 0;
        
        for (uint256 i = 0; i < allActivityIds.length; i++) {
            if (hasCompleted[allActivityIds[i]][avatarId]) {
                completed[count] = allActivityIds[i];
                count++;
            }
        }
